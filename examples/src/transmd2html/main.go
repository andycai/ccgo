...
func (self *Visitor) visit(path string, f os.FileInfo, err error) error {
	...
	if f.IsDir() {
		...
		if strings.HasSuffix(f.Name(), ".md") {
			...
			output = append([]byte("<!DOCTYPE html><html><head><meta charset='utf-8'><title>An Introduction to Programming in Go</title></head><body>"), output...)
			output = append(output, []byte("</body></html>")...)
